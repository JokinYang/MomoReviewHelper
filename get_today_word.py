...
def main():
    ...
    # if main page exists, there is not need to login.
    if not d(resourceId=ID.main_page_container).exists(TIMEOUT):
        try:
        ...
        print('Main page exists,skip login')
    # Check login status
    if not d(resourceId=ID.main_page_container).exists(60):
    ...
    if d(resourceId=ID.ok_button).exists(timeout=20):
        print('It is ok !!!')
    else:
